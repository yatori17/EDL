...
avalia (Mai a b)= if(avalia a > avalia b) then 1 else 0
avalia (Eq a b)= if(avalia a == avalia b) then 1 else 0
...
avalia' (Sub a b)= Num (f (avalia' a)(avalia' b)) where
    f (Num x) (Num y)= x - y
avalia' (Mai a b)= Num (f (avalia' a)(avalia' b)) where
    f (Num x) (Num y)= if ( x > y) then 1 else 0
avalia' (Eq a b)= Num (f (avalia' a)(avalia' b)) where
    f (Num x) (Num y)= if x ==y then 1 else 0
